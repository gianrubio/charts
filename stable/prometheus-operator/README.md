...
| `prometheus.prometheusSpec.image.tag` | Tag of Prometheus container image to be deployed. Defaults to the value of `version`. Version is ignored if Tag is set. | v2.3.4 |
...
| `prometheus.prometheusSpec.retention` | Time duration Prometheus shall retain data for. Default is '24h', and must match the regular expression `[0-9]+(ms\|s\|m\|h\|d\|w\|y)` (milliseconds seconds minutes hours days weeks years). | 120h |
...
| `alertmanager.alertmanagerSpec.image.tag` | Tag of Alertmanager container image to be deployed. Defaults to the value of `version`. Version is ignored if Tag is set. | 0.15.2 |
